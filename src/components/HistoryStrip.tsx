  ...
  return (
    <div>
        ...
        {entries.map((entry, i) => {
          ...
          return (
            <Fragment key={`${entry.month}-${i}`}>
              ...
              <li className="flex shrink-0 flex-col items-center" style={{ width: s(32) }}>
                <span
                  className={[
                    'tnum flex items-center justify-center rounded-pill font-bold',
                    isLatest ? 'bg-ink text-paper' : 'bg-paper text-ink',
                  ].join(' ')}
                  style={{ width: s(32), height: s(32), fontSize: s(12), lineHeight: 1 }}
                >
                  {entry.stars}
                </span>
                <span
                  className="text-slate"
                  style={{ fontSize: s(11), lineHeight: s(16), marginTop: s(4) }}
                >
                  {entry.month}
                </span>
              </li>
      ...
      {entries.length === 1 ? (
        ...
        <p className="text-slate" style={{ fontSize: s(11), lineHeight: s(16), marginTop: s(6) }}>
          First review
        </p>
      ) : null}
    </div>
